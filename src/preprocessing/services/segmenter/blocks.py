...
"""Block-level Markdown scanning utilities for the segmenter service.

This module detects fenced code blocks, headings, paragraphs, lists, blockquotes,
HTML blocks, and pipe tables. It emits non-overlapping Block records with
absolute spans used by inline extraction.
"""
...
class Block:
    ...
    Attributes:
        kind: Block kind (heading, paragraph, list_item, blockquote, table,
...
class BlockScanner:
    ...
    Provides scan_blocks() to iterate document blocks and iter_table_cells() to
    produce per-cell blocks inside a table block.
    """
    ...
    def scan_blocks(self) -> Iterator[Block]:
        """Yield non-overlapping blocks in source order."""
        i = 0
        n = len(self._lines)
        md = self._md
        while i < n:
            ...
            if m_fence:
                ...
                logger.debug("code fence %r at %d..%d", fence, abs_start, abs_end)
            ...
            if line.startswith("    ") or line.startswith("\t"):
                ...
                logger.debug("indented code at %d..%d", abs_start, abs_end2)
            ...
            if self._re_html_start.match(line):
                ...
                logger.debug("html block <%s> at %d..%d", tag, abs_start, abs_end3)
            ...
            if m_h:
                ...
                logger.debug("heading #%d at %d", level, abs_start)
                yield Block(
                    "heading",
                    abs_start,
            ...
            if m_bq:
                ...
                logger.debug("blockquote at %d..%d", abs_start, abs_end5)
            ...
            if m_ul or m_ol:
                ...
                logger.debug("list item at %d", abs_start)
                yield Block(
                    "list_item",
                    abs_start,
                    self._line_starts[i + 1] if i + 1 <= n else len(md),
            ...
            logger.debug("paragraph at %d..%d", abs_start, abs_end6)
...
def _compute_line_starts(lines: Sequence[str]) -> List[int]:
    """Compute absolute start offsets for each line (plus sentinel at end)."""
    starts: List[int] = [0]
    total = 0
    for ln in lines:
        total += len(ln)
...
def line_starts_with(line: str, fence: str) -> bool:
    """Check if a line starts with a code fence allowing leading whitespace."""
...
def _split_table_row_cells(line: str) -> List[Tuple[int, int]]:
    """Split a table row into raw cell spans (excluding pipes)."""
