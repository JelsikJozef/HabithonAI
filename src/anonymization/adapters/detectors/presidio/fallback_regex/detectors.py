...
from .patterns import CREDIT_CARD_RE, EMAIL_RE, IPV4_RE, PHONE_RE, luhn_valid
...
def detect_with_regex(text: str, detector_name: str = "presidio") -> list[PiiEntity]:
    ...
    for m in EMAIL_RE.finditer(text):
        ...
        out.append(
            PiiEntity(
                type="EMAIL", start=s, end=e, value=m.group(0), score=0.6, detector=detector_name
            )
        )
    for m in PHONE_RE.finditer(text):
        ...
        out.append(
            PiiEntity(
                type="PHONE", start=s, end=e, value=m.group(0), score=0.5, detector=detector_name
            )
        )
    for m in IPV4_RE.finditer(text):
        ...
        s, e = m.span()
        out.append(
            PiiEntity(type="IP", start=s, end=e, value=val, score=0.5, detector=detector_name)
        )
